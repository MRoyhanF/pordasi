...
<div class="bg-gradient-to-r from-green-600 to-green-800 text-white p-4 sm:p-6 lg:p-8">
    <div class="flex items-center justify-between mb-2">
        <h2 class="text-xl sm:text-2xl lg:text-3xl font-bold">Data Pengguna</h2>
        <button class="create-btn px-4 py-2 bg-white text-green-600 rounded-lg hover:bg-gray-100 transition text-sm font-medium">
            + Tambah Pengguna
        </button>
    </div>
    <p class="text-green-100 text-sm sm:text-base">Kelola dan lihat data pengguna sistem</p>
</div>
...
<div class="p-4 sm:p-6 lg:p-8 flex flex-col">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden flex flex-col">
        <table id="penggunaTable" class="w-full">
            <thead>
                <tr>
                    <th class="text-left px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b">No</th>
                    <th class="text-left px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b">Nama</th>
                    <th class="text-left px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b">Email</th>
                    <th class="text-center px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b">Role</th>
                    <th class="text-center px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b">Terdaftar</th>
                    <th class="text-center px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b">Aksi</th>
                </tr>
            </thead>
            <tbody id="tableBody">
                @forelse($users as $item)
                <tr class="hover:bg-gray-50 border-b border-gray-100 transition" data-role="{{ $item->role }}">
                    ...
                    <td class="px-4 py-3 text-center">
                        @php
                            ...
                            $color = $roleColors[$item->role] ?? 'bg-gray-100 text-gray-800';
                        @endphp
                        <span class="inline-block px-3 py-1 {{ $color }} rounded-full text-xs font-semibold">
                            {{ $item->role ?? '-' }}
                        </span>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center text-gray-500 py-8 text-sm">Tidak ada data pengguna</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between px-4 py-4 bg-gray-50 border-t border-gray-100 gap-4">
            <div class="text-sm text-gray-600">
                Menampilkan <span id="startItem" class="font-semibold">1</span> hingga <span id="endItem" class="font-semibold">10</span> dari <span id="totalItems" class="font-semibold">{{ count($users) }}</span> data
            </div>
...
@include('pengguna.partials.modal-create-pengguna')
@include('pengguna.partials.modal-edit-pengguna')
@include('pengguna.partials.modal-delete-pengguna')

@vite('resources/js/pengguna/init.js')
